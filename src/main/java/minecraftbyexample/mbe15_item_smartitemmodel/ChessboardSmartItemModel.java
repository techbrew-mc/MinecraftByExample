 ...
 *
 *   TODO:  Lots of 1.9.4 changes here, need to review
 */
public class ChessboardSmartItemModel implements IBakedModel{
  ...
  public static final ModelResourceLocation modelResourceLocation = new ModelResourceLocation("minecraftbyexample:mbe15_item_chessboard", "inventory");

  private final ChessBoardHandler chessBoardHandler;
  ...
  {
    chessBoardHandler = new ChessBoardHandler(i_baseChessboardModel);
  ...
  {
    List<BakedQuad> combinedQuadsList = new ArrayList<BakedQuad>(baseChessboardModel.getQuads(state, side, rand));
    combinedQuadsList.addAll(chessBoardHandler.getChessPiecesQuads());
  ...
  {
    return chessBoardHandler;
  }

  class ChessBoardHandler extends ItemOverrideList
  {

    private int numberOfChessPieces;

    public ChessBoardHandler(IBakedModel baseChessboardModel)
    {
      super(baseChessboardModel.getOverrides().getOverrides());
    }
    ...
    public IBakedModel handleItemState(IBakedModel originalModel, ItemStack stack, World world, EntityLivingBase entity)
    {
      ...
      return baseChessboardModel;
    }

    // return a list of BakedQuads for drawing the chess pieces
    public List<BakedQuad> getChessPiecesQuads()
    {
      ...
      //        event.getMap().registerSprite(digits);
      //      }
      //    }

      List<BakedQuad> returnList = new ArrayList<BakedQuad>(64);
      if (numberOfChessPieces < MIN_NUMBER_OF_PIECES || numberOfChessPieces > MAX_NUMBER_OF_PIECES) {
        return returnList;
      }
